    ...
    <div class="row">
            ...
            <div class="card">
                ...
                <div class="card">
                    <div class="card-body border-bottom">
                        <form action="{{ route('students.clubs.resources.store', $club->clubID) }}" method="POST" enctype="multipart/form-data">
                            @csrf
        ...
        <div class="col-12 col-xl-9">
            <div class="card">
                <div class="card-body">
                    <div class="mb-3">
                        <a href="{{ route('students.clubs.resources', $club->clubID) }}" class="btn btn-sm btn-outline-secondary {{ request('type') === null ? 'active' : '' }}">All</a>
                        <a href="{{ route('students.clubs.resources', [$club->clubID, 'type' => 'word']) }}" class="btn btn-sm btn-outline-primary {{ request('type') === 'word' ? 'active' : '' }}">Word</a>
                        <a href="{{ route('students.clubs.resources', [$club->clubID, 'type' => 'pdf']) }}" class="btn btn-sm btn-outline-danger {{ request('type') === 'pdf' ? 'active' : '' }}">PDF</a>
                        <a href="{{ route('students.clubs.resources', [$club->clubID, 'type' => 'image']) }}" class="btn btn-sm btn-outline-warning {{ request('type') === 'image' ? 'active' : '' }}">Images</a>
                        <a href="{{ route('students.clubs.resources', [$club->clubID, 'type' => 'video']) }}" class="btn btn-sm btn-outline-info {{ request('type') === 'video' ? 'active' : '' }}">Videos</a>
                        <a href="{{ route('students.clubs.resources', [$club->clubID, 'type' => 'zip']) }}" class="btn btn-sm btn-outline-dark {{ request('type') === 'zip' ? 'active' : '' }}">ZIP</a>
                    </div>
                    <form method="GET" action="{{ route('students.clubs.resources', $club->clubID) }}" class="mb-3 d-flex" role="search">
                    ...
                    <div class="list-group">
                        ...
                        <div class="list-group-item d-flex justify-content-between align-items-center">
                            ...
                            <div class="d-flex gap-2 align-items-center">
                                ...
                                @if($resource->userID === auth()->id())
                                <form action="{{ route('students.clubs.resources.destroy', [$club->clubID, $resource->resourceID]) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this file?');">
                                    @csrf
                                    @method('DELETE')
